...
데이터베이스 커넥션 풀, 로깅, 설정 관리 같은 객체가 애플리케이션 여러 곳에서 `new`로 반복 생성되면 심각한 문제가 발생한다. 동일한 역할의 객체가 메모리에 여러 개 존재하면서 메모리가 낭비되고,
한쪽에서 값을 변경해도 다른 쪽에서는 반영되지 않는 상태 불일치가 생기며, 파일 I/O나 네트워크 연결 같은 비용이 큰 초기화가 반복된다.
