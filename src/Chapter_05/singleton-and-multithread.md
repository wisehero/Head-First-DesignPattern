...
두 번째 질문에 대한 전략은 세 가지로 나뉜다. **상태를 없애거나(stateless)** — Spring의 `@Service`가 보통 이 방식이다. 비즈니스 로직만 있고 인스턴스 변수에 상태를 저장하지 않는다. *
*상태를 불변으로 만들거나(immutable)** — 생성 이후 변경이 없으면 동기화 자체가 불필요하다. **동기화된 자료구조를 사용하거나(thread-safe collections)** — 상태 변경이 불가피하다면
